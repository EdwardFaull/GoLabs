...
func worker(startY, endY, startX, endX int, data func(x, y int) uint8, out chan<- [][]uint8){
	newPixelData := medianFilter(startY, endY, startX, endX, data)
	out <- newPixelData
}
...
func filter(filepathIn, filepathOut string) {
	...
	for i := 0; i < divisions; i++{
		...
		go worker(i * height / divisions, (i + 1) * height / divisions, 0, width, immutableData, channel)
		results[i] = channel
	}

	newPixelData := make([][]uint8, height, width)

	for slices := 0; slices < 4; slices += 0{
		for i := 0; i < 4; i++{
			select{
				case slice := <-results[i]:
...
func oldFilter(filepathIn, filepathOut string){
	image.RegisterFormat("png", "PNG", png.Decode, png.DecodeConfig)
	image.RegisterFormat("jpeg", "jpeg", jpeg.Decode, jpeg.DecodeConfig)
