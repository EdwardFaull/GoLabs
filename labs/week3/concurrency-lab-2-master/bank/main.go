...
func manager(bank *bank, transactionQueue <-chan transaction) {
	for i := 0; i < 1000; i++ {
...
func executor(bank *bank, executorId int, transactionQueue chan transaction, done chan<- bool) {
	for {
		...
		if !bank.lockAccount(t.to, strconv.Itoa(executorId)) {
			bank.unlockAccount(t.from, strconv.Itoa(executorId))
			//fmt.Println("Executor\t", executorId, "unlocked account", from, ", account busy")
			transactionQueue <- t
		} else {
			...
			bank.unlockAccount(t.from, strconv.Itoa(executorId))
			//fmt.Println("Executor\t", executorId, "unlocked account", from, ", transaction completed")
			bank.unlockAccount(t.to, strconv.Itoa(executorId))
			//fmt.Println("Executor\t", executorId, "unlocked account", to, ", transaction completed")
